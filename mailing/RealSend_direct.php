<?php
	...
	include ("RealSend_function.php"); // Send Function
	...
	{
		$IPS[$count]= trim($ips); // trim IP from any space
		...
		$requeteIP->execute(array($IPS[$count]));
		
		$rowIP   = $requeteIP->fetch();	
		$domain[$count]    = $rowIP['name_Domain'];
		$idIP[$count]      = $rowIP['id_IP'];
		$aliasServer[$count] = $rowIP['alias_Server']; 
		$count++;
		}
	...
	for ($i=1;$i<=$repeat;$i++){
		// For each IP
		for($x=0;$x<=count($IPS)-1;$x++)
		{
			Send($idISP,$host,$xDelay,$id_Send,$isSender,$idFrom,$subject,$creative,$id_negative,$IPS[$x],$idIP[$x],
			$aliasServer[$x],$domain[$x],$emailsTests,$seed,$cptSeed,$tableName,$startFrom,$fraction,$headerOriginal,$bodyOriginal,$returnPathOriginal);
			/*if ($idISP!=3) {
				if($isAR == 0){
					Send($host,$xDelay,$id_Send,$isSender,$idFrom,$subject,$creative,$id_negative,$ip,$idIP[$x],$aliasServer[$x],$domain[$x],$emailsTests,$seed,$cptSeed,$tableName,$startFrom,$fraction,$headerOriginal,$bodyOriginal,$returnPathOriginal);
				}
				$startFrom+=$fraction;
			}else {
				if($isAR == 0){
					Send_Warmup($host,$xDelay,$id_Send,$isSender,$idFrom,$subject,$creative,$id_negative,$ip,$idIP[$x],$aliasServer[$x],$domain[$x],$emailsTests,$seed,$cptSeed,$tableName,$startFrom,$fraction,$headerOriginal,$bodyOriginal,$returnPathOriginal);
				}
			}*/
			echo "$IPS[$x] => $fraction emails in $time seconds\n";
		}
	}
