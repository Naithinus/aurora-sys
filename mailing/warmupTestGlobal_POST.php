...
if($rotation==true)
{
	...
	for($y=0;$y<count($tab_ips);$y++)
	{
	 ...
	 if(empty($fraction)){
		 $fraction=count($tab_ips)*count($emailTests);
	 }
	if($fraction>count($tab_ips)*count($emailTests)){
	 echo "Error: Fraction bigger than the number of emails!";
	 die();
	 }
	for($y=0;$y<count($tab_ips);$y++)
	{
