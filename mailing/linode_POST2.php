...
set_include_path(get_include_path() . PATH_SEPARATOR . 'SSH2');
$password="NpdFw8gbf@@!2@17";
if(isset($_POST["name"]))
	{
		...
		for($i=1;$i<=$quantity;$i++){
			...
			if(!empty($linode_id)){
				...
				$newServer=$bdd->exec("insert into server values(NULL,'$linode_name',30,'root','$password',now(),now(),1,NULL,1,NULL,NULL,NULL)");
	...
	{
		$linode_id=$_GET["delete"];
		$linode_name=$_GET["name"];
		$ch = curl_init();
	...
	if(isset($_GET["config"])){
		...
		"yum -y install net-tools|tee -a /tmp/install_log",
		"yum -y install unzip",
		"yum install -y httpd httpd-devel httpd-manual httpd-tools mod_auth_kerb mod_auth_mysql mod_auth_pgsql mod_authz_ldap mod_dav_svn mod_dnssd mod_nss mod_perl mod_revocator mod_ssl mod_wsgi",
		"cd /home;wget 79.143.189.72/PowerMTA-4.0r6-201204021809.x86_64.rpm;wget 79.143.189.72/license",
		"cd /etc;wget -N 79.143.189.72/sudoers;wget -N 79.143.189.72/php.ini",
		"cd /etc/httpd/conf;wget -N 79.143.189.72/httpd.conf",
		"cd /var/www;wget -N 79.143.189.72/_var_www.zip;ls -l",
		"unzip -o /var/www/_var_www.zip -d /var/www",
		"hostname carlasmir.club",
		"echo SERVER_IP  carlasmir.club> /etc/hosts",
		"sed -i -e 's/$ip/SERVER_IP/g' /etc/httpd/conf/httpd.conf",
		"rpm -ivh /home/PowerMTA-4.0r6-201204021809.x86_64.rpm",
		"cp /home/license /etc/pmta/license",
		...
		//"sed -i -e 's/domain-key/#domain-key/g' /var/www/exactarget/PMTA/vmta.txt",
		...
		uploadFile($sourceFile,$targetFile,$main_ip,$login,$password,0777);
		 ...
		 $ssh->exec("sed -i -e 's/domain-key/#domain-key/g' /var/www/exactarget/PMTA/vmta.txt");
		 $ssh->exec("pmta reload");
		 $ssh->exec("service pmta restart");
		 $ssh->exec("service httpd restart");
		header("location:./");
