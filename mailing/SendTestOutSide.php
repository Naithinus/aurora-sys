			...
			foreach($explodeEmailTest as $emailTest)
			{
						...
						if($id_negative != 0)
						{
							$body =      preg_replace('#\[nega\]#',$contentNegative,$body);
							  ...
							  // $idEmail = 0;
							  $requeteSend = $bdd->query("select id_Email from email where email_Email=\"$emailTest\" ");
							  while($rowSend = $requeteSend->fetch())
								{
									$idEmail = $rowSend['id_Email'];
								}
							   $XID_query=$bdd->query("Select password_email from email_list_warmup where id_email=\"$idEmail\"");
				               ...
				               $fname = $fnamea[0];
							    $edomain = $fnamea[1];
							   $header =      preg_replace('#\[edomain\]#',$edomain,$header);
							   ...
							   $header =    preg_replace('#\[X-smfbl\]#',base64_encode($to),$header);
			                   $header =    preg_replace('#\[fname\]#',$fname,$header);
							   ...
							   $body =      preg_replace('#\[fname\]#',$fname,$body);
						...
						foreach($out[0] as $random)
						{
						   ...
						   $returnPath = preg_replace('#\[domain\]#',$domain,$returnPath);
						     $words = preg_split('//', 'abcdefghijklmnopqrstuvwxyz', -1);
							   ...
							   $returnPath = preg_replace('#\[rand\]#',$rand,$returnPath);
						   $idEmail = 0;
